<?php
	...
	if (!empty($_POST))  {
		...
		$query = "Select category, lat, lng FROM `not_confirmed` WHERE id = :id";
...
?>
		<h1>Fetch Not Confirmed Added Marker</h1>
		<form action="fetchnotconfirmedaddedmarker.php" method="post">
			ID do dispositivo: <br />
			<input type="int" name="id" placeholder="id do dispositivo" value="" />
			<br></br>
			<input type="submit" value="Fetch Not Confirmed Added Marker" />
		</form>
<?php
	}
?>